...
public class Describe {
	...
	public static String sizePercent(long saved, long size) {
		...
		if (size == -1) {
			...
			if (saved == 0) return "";
			else            return "Saved " + Describe.size(saved);
		...
		// We know the size
			...
			// Both saved and size must be 1 or more
				...
				if (percent == 0) return Describe.size(saved) + "/" + Describe.size(size); // Omit 0%
				else if (percent == 100) return Describe.size(size); // Omit 100% and the saved size
				else return percent + "% " + Describe.size(saved) + "/" + Describe.size(size);
			}
		}
	}
