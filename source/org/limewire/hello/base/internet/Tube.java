...
public class Tube {
	 ...
	 */
	public Tube(InternetSelect select, SocketChannel channel) {
		...
		upload = new TubeUpload(this);
		download = new TubeDownload(this);
		attempt = new Time();
		connect = new Time();
		...
		} catch (IOException e) { close(new State(State.socketException, e)); return; }
	 ...
	 */
	public Tube(InternetSelect select, IpPort address) {
	...
	public State state() {
		...
		if (channel.isConnected()) return State.doing();
		else                       return State.opening();
	}
	...
	public Time response() {
		...
		return Time.recent(connect, download.transfer);
	}
