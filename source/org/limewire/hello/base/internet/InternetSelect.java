			...
			try {
				...
				while (i.hasNext()) {
					...
					if (server != null && key.channel() == server) {
						...
						if (key.isAcceptable()) new Tube(null, server.accept()); // Adds the new Tube to the list
					...
					// It must be a TCP socket connection in a Tube object
						...
						Tube tube = (Tube)key.attachment(); // Get the Tube the channel is in
						if (!tube.state().isClosed()) {     // Don't touch a closed Tube
							...
							} catch (IOException e) { tube.close(new State(State.socketException, e)); }
						}
					}
