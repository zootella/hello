...
import org.limewire.hello.base.state.Spin;

public class OldUpdate extends OldClose {
	
	//TODO split this into three classes: Update, Pulse, and Model, have them all in the same package
	// Update here is really simple, just updates, no grouping or pulsing, no timer
	// Another one, called Pulse, just pulses, you could hook an Update and a Pulse up to the same UpdateReceive
	// I think the only place you need the delay is in Model and View, and it can be on by default, and only this one has sendNow for final update
	 ...
	 */
	public OldUpdate(Receive receive) {
